...
def one_dcnn(n_filters, kernel_size, input_array, initializer):
    ...
    cnn.add(Conv1D(filters=n_filters, kernel_size=kernel_size, kernel_initializer=initializer, padding='same', groups= 10, input_shape=(input_array.shape[1],input_array.shape[2])))
    # cnn.add(BatchNormalization())
    cnn.add(Activation('relu'))
    cnn.add(Conv1D(filters=n_filters, kernel_size=kernel_size, kernel_initializer=initializer, padding='same', groups= 10))
    # cnn.add(BatchNormalization())
    cnn.add(Activation('relu'))
    cnn.add(Conv1D(filters=1, kernel_size=kernel_size, kernel_initializer=initializer, padding='same', groups= 10))
    # cnn.add(BatchNormalization())
    cnn.add(Activation('relu'))
    cnn.add(Flatten())
